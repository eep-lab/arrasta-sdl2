...
{
  Imported SpeechLib on 13/09/2023 15:59:34 from C:\WINDOWS\System32\Speech\Common\sapi.dll
  Step 1: Make sure you have SAPI installed.
  Step 2: Install LazActivex package
  Step 3: On Lazarus, Tools -> Import Type Library
  Step 4: Select the tab ActiveX References
  Step 5: Select Microsoft Speech Object Library on the list
  Step 6: Save the file
  Step 7: Add the following code at EOF
  ...
  check example use cases here: src\picanco.experiments.audio.pas;
}
...
//CoClasses
  ...
  Public
    ...
    property ComServer:ISpeechVoice read fServer;
  ...
  TEvsSpSharedRecoContext = Class(TEventSink)
  Private
  ...
  Public
    constructor Create(TheOwner: TComponent); override;
    property ComServer:ISpeechRecoContext read fServer;
  ...
  TEvsSpInProcRecoContext = Class(TEventSink)
  Private
    FOnStartStream:T_ISpeechRecoContextEventsStartStream;
    FOnEndStream:T_ISpeechRecoContextEventsEndStream;
    FOnBookmark:T_ISpeechRecoContextEventsBookmark;
...
constructor TEvsSpVoice.Create(TheOwner: TComponent);
begin
  inherited Create(TheOwner);
  OnInvoke:=EventSinkInvoke;
  fServer:=CoSpVoice.Create;
  Connect(fServer,_ISpeechVoiceEvents);
end;

procedure TEvsSpVoice.EventSinkInvoke(Sender: TObject; DispID: Integer;
...
constructor TEvsSpSharedRecoContext.Create(TheOwner: TComponent);
begin
  inherited Create(TheOwner);
  OnInvoke:=EventSinkInvoke;
  fServer:=CoSpSharedRecoContext.Create;
  Connect(fServer,_ISpeechRecoContextEvents);
end;

procedure TEvsSpSharedRecoContext.EventSinkInvoke(Sender: TObject; DispID: Integer;
  const IID: TGUID; LocaleID: Integer; Flags: Word; Params: tagDISPPARAMS;
  VarResult, ExcepInfo, ArgErr: Pointer);
begin
  case DispID of
...
constructor TEvsSpInProcRecoContext.Create(TheOwner: TComponent);
begin
  inherited Create(TheOwner);
  OnInvoke:=EventSinkInvoke;
...
procedure TEvsSpInProcRecoContext.EventSinkInvoke(Sender: TObject; DispID: Integer;
  const IID: TGUID; LocaleID: Integer; Flags: Word; Params: tagDISPPARAMS;
  VarResult, ExcepInfo, ArgErr: Pointer);
begin
  case DispID of
